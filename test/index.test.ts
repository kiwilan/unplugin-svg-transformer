...
describe('svgPlugin', () => {
  it('can get the paths', () => {
    ...
    // expect(paths.iconsDir).toBe(`${process.cwd()}/tests/icons/svg`)
    // expect(paths.cacheDir).toBe(`${process.cwd()}/tests/icons/cache`)
    // expect(paths.filenamePath).toBe(`${process.cwd()}/tests/icons/icons.ts`)
    // expect(paths.gitignorePath).toBe(`${process.cwd()}/tests/icons/.gitignore`)
  ...
  //     expect(twitter?.getFullPath().includes('tests/icons/svg/social/twitter.svg')).toBe(true)
  ...
  //     const files = await SvgItem.toList(getPaths().iconsDir, getPaths().iconsDir)
  //     await FileUtils.writeSvgAsTs(files, getPaths().cacheDir)
  ...
  //     const files = await SvgItem.toList(getPaths().iconsDir, getPaths().iconsDir)
  //     await TsConverter.make(files, getPaths().iconsDir, getPaths().cacheDir, getPaths().filenamePath)
  ...
  //     await fs.rm(getPaths().gitignorePath, { force: true })

  //     // create `.gitignore` file
  //     await fs.writeFile(getPaths().gitignorePath, '')

  //     await FileUtils.addPathToGitignoreIfNotExists(getPaths().cacheDir, getPaths().gitignorePath)
  //     const content = await FileUtils.read(getPaths().gitignorePath)
